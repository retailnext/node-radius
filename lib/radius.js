...
Radius._decode = function(args) {
  ...
  if (!uses_random_authenticator[ret.code] && is_request_code[ret.code]) {
  ...
  if (is_request_code[ret.code] && ret.attributes["Message-Authenticator"]) {
    return this._verify_request_message_authenticator(args, ret);
  } else {
    if (args.callback) {
...
Radius._verify_request_message_authenticator = function(args, request) {
  ...
  var calculate_and_verify = function(raw_packet) {
    request.authenticator.copy(raw_packet, AUTH_START);
    var orig_ma = request.attributes["Message-Authenticator"];
    var expected_ma = self.calculate_message_authenticator(raw_packet, args.secret);
    if (orig_ma.toString() != expected_ma.toString()) {
      return new Radius.InvalidSecretError("decode: Message-Authenticator mismatch (possible shared secret mismatch)", request);
    } else {
      return;
    }
  };
  ...
  if (args.callback) {
    callback = function(err, encoded) {
      if (err) {
        self.error("decode: error while verifying message authenticator: " + err.message,
                   args.callback);
        return;
      }

      err = calculate_and_verify(encoded);
      if (err) {
        self.error(err, args.callback);
      } else {
        args.callback(null, request);
      }
    };
  }
  ...
  var reencoded = this.encode({
    code: request.code,
    attributes: self.zero_out_message_authenticator(request.raw_attributes),
  ...
  if (!args.callback) {
    var err = calculate_and_verify(reencoded);
    if (err) {
      self.error(err);
    } else {
      return request;
...
Radius.verify_response = function(args) {
  ...
  if (args.callback) {
    self._verify_response_message_authenticator_async(args);
    return;
  } else {
    return self._verify_response_message_authenticator_sync(args);
  }
};

Radius._verify_response_message_authenticator_sync = function(args) {
...
Radius._verify_response_message_authenticator_async = function(args) {
  var self = this;

  self.decode({
    packet: args.request,
    secret: args.secret,
    callback: function(err, parsed_request) {
      if (err) {
        self.error("verify_response: error decoding request: " + err.message, args.callback);
        return;
      }
      ...
      self.decode({
        packet: args.response,
        secret: args.secret,
        callback: function(err, parsed_response) {
          if (err) {
            self.error("verify_response: error decoding response: " + err.message, args.callback);
            return;
          }
          ...
          self.encode({
            ...
            code: parsed_response.code,
            identifier: parsed_response.identifier,
            attributes: self.zero_out_message_authenticator(parsed_response.raw_attributes),
            callback: function(err, expected_response) {
              if (err) {
                self.error("verify_response: " + err.message, args.callback);
                return;
              }

              parsed_request.authenticator.copy(expected_response, AUTH_START);
              var expected_ma = self.calculate_message_authenticator(expected_response, args.secret);
              args.callback(null, expected_ma.toString() == got_ma.toString());
            }
          });
        }
      });
...
Radius._encode_response = function(args) {
  ...
  var proxy_state_id = self.attr_name_to_id("Proxy-State");
