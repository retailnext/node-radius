...
Radius._decode = function(args) {
  ...
  if (ret.code != "Access-Request" && ret.code.match(/Request/)) {
    var orig_authenticator = new Buffer(16);
    packet.copy(orig_authenticator, 0, 4, 20);
    packet.fill(0, 4, 20);

    var checksum = this.calculate_packet_checksum(packet, args.secret);
    orig_authenticator.copy(packet, 4);
...
Radius._encode_with_authenticator = function(args, packet, offset, authenticator) {
  ...
  if (args.code != "Access-Request") {
    this.calculate_packet_checksum(packet, args.secret).copy(packet, 4);
  }
