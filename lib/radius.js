...
Radius.load_dictionary = function(path, callback) {
  if (callback) {
    ...
    return this._load_dictionary(fs.readFileSync(path, 'ascii'));
  }
};
...
Radius._decode = function(args) {
  ...
  if (!packet || packet.length < 4)
    throw new Error("decode: invalid packet");
...
Radius._encode_response = function(args) {
  var packet = args.packet;
  if (!packet)
    throw new Error("encode_response: must provide packet");
...
Radius._encode = function(args) {
  ...
  if (identifier > 255)
    throw new Error("encode: identifier too large (must be in the range 0-255)");
