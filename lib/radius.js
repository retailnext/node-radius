...
Radius.decode = function(args) {
  ...
  ret.attributes = {};
  ret.raw_attributes = [];
  ret.raw_vendor_attributes = {};

  try {
    this.decode_attributes(attrs, ret.attributes, NO_VENDOR, ret.raw_attributes, ret.raw_vendor_attributes);
  } catch(err) {
    this.error(err);
    return;
  }
...
Radius.decode_attributes = function(data, attr_hash, vendor, raw_attrs, raw_vendor_attrs) {
  ...
  while (data.length > 0) {
    ...
    if (vendor !== NO_VENDOR && raw_vendor_attrs) {
        raw_vendor_attrs[vendor] = raw_vendor_attrs[vendor] || [];

        raw_vendor_attrs[vendor].push([type, value]);
    }
    ...
    if (attr_info[ATTR_MODIFIERS]["encrypt=1"]) {
      ...
      if (attr_info[ATTR_NAME] == "Vendor-Specific") {
        ...
        this.decode_attributes(value.slice(4), vendor_attrs, value.readUInt32BE(0), null, raw_vendor_attrs);
        continue;
      }
    }
