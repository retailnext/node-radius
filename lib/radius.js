...
Radius.load_dictionaries = function(callback) {
  ...
  for (var i = 0; i < dictionary_locations.length; i++) {
    var file = dictionary_locations[i];
...
Radius.load_dictionary = function(file, callback, seen_files) {
  ...
  if (callback) {
    fs.readFile(file, "ascii", function(err, contents) {
      if (err) throw err;
      var includes = this._load_dictionary(contents);
      callback(includes.length);
      includes.forEach(function (i) {
        this.load_dictionary(path.join(path.dirname(file), i), callback, seen_files);
      }.bind(this));
    }.bind(this));
  } else {
    var includes = this._load_dictionary(fs.readFileSync(file, "ascii"));
    includes.forEach(function (i) {
      this.load_dictionary(path.join(path.dirname(file), i), callback, seen_files);
    }.bind(this));
  }
